...
export default function AdminDashboard({ reports = [], language = 'en', onNavigate, onSelectArea }) {
  ...
  const ucCount = reports.filter((r) => ((r.reportType || r.type || '').includes('Condition'))).length;
  const ueCount = reports.filter((r) => ((r.reportType || r.type || '').includes('Event') || (r.reportType || r.type || '').includes('Act'))).length;
  const criticalCount = reports.filter((r) => r.severity === 'Critical' || r.aiAnalysis?.sifPotential === 'Yes').length;
  ...
  const reportsToday = reports.filter((r) => ((r.eventDate || r.date || '').includes(todayStr))).length;

  // Chart 1: Reports by Area
  const areaCounts = {};
  reports.forEach((r) => {
    const rawArea = r.area || 'General';
    const mainArea = rawArea.split('/')[0].trim();
    ...
    <div>
      <div className="page-header-row">
        ...
        <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
          <button className="btn btn-gold" onClick={() => onNavigate('ai-dashboard')}>
            <Cpu size={16} />
            AI SIF Precursor Analytics
          </button>
          <button className="btn btn-secondary" onClick={() => onNavigate('reports-by-area')}>
            <Layers size={16} />
            Area-wise Bifurcation
          </button>
